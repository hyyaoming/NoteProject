...
public abstract class AbstractSlideExpandableListAdapter extends WrapperListAdapterImpl {
	 ...
	 * 记录上一个点击位置
	 */
	private View lastContentView = null;
	 ...
	 *
	 * @param wrapped  ListAdapter
	 * @param position int表示默认展开的item的位置
	 */
	...
	private void notifiyExpandCollapseListener(int type, View view, int position,View togglebuttonView) {
		if (expandCollapseListener != null) {
			if (type == ExpandCollapseAnimation.EXPAND) {
				expandCollapseListener.onExpand(view, position,togglebuttonView);
			} else if (type == ExpandCollapseAnimation.COLLAPSE) {
				expandCollapseListener.onCollapse(view, position,togglebuttonView);
			}
		}
	}
	 ...
	 * @return View
	 */
	public abstract View getArrowId(View parent);
	 ...
	 * 关闭上一个点击位置arrow
	 */
	private void resetLastArrow() {
		if (isAnyItemExpanded()) {
			if (null != lastContentView) {
				View arrow = getArrowId(lastContentView);

				if (null != mContext) {
					arrow.startAnimation(AnimationUtils.loadAnimation(mContext, R.anim.arrow_rotate_down));
				}
	...
	public void enableFor(View parent, int position) {
		...
		View arrow = getArrowId(parent);
		View itemToolbar = getExpandableView(parent);
		View toggleButtonView = getExpandToggleButton(parent);
		/**
		 * in case of nullpointer
		 * due to :multi-listview item xml style
		 */
		if (null != itemToolbar) {
			itemToolbar.measure(parent.getWidth(), parent.getHeight());

			enableForSingle(more, itemToolbar, position, arrow,toggleButtonView);
	 ...
	 * @param arrow    View
	 */
	private void enableFor(final View button, final View target, final int position, final View arrow,final View togglebuttonView) {
		mContext = getContext();
...
//            lastOpen = target;
			// expand item
			// set arrow status
			if (null != mContext&&!arrow.isSelected()) {
				arrow.startAnimation(AnimationUtils.loadAnimation(mContext, R.anim.arrow_rotate_right_up));
				arrow.setSelected(true);
//            lastContentView = arrow;
				notifiyExpandCollapseListener(ExpandCollapseAnimation.EXPAND, target, position,togglebuttonView);
			}

		}else if(!openItems.get(position)){
			arrow.setAnimation(null);
			arrow.setSelected(false);
			notifiyExpandCollapseListener(ExpandCollapseAnimation.COLLAPSE, target, position,togglebuttonView);
		}
...
//                        lastOpen = target;
					...
					notifiyExpandCollapseListener(type, target, position,togglebuttonView);
				}
			}
		});
	}
	 ...
	 * @param arrow    View
	 */
	private void enableForSingle(final View button, final View target, final int position, final View arrow,final View togglebuttonView) {
		mContext = getContext();
					...
					if (type == ExpandCollapseAnimation.EXPAND) {
						if (lastOpenPosition != -1 && lastOpenPosition != position) {
							if (lastOpen != null) {
								...
								notifiyExpandCollapseListener(
										ExpandCollapseAnimation.COLLAPSE,
										lastOpen, lastOpenPosition,togglebuttonView);
							}
							openItems.set(lastOpenPosition, false);
						}
						lastOpen = target;
